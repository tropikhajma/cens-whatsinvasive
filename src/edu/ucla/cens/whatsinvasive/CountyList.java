...
public class CountyList extends ListActivity implements Observer {
	...
	private static final String TAG = "CountyList";

	protected static final int RESULT_TAGS_SAME = 22;
	
	private final int VISIBLE_COUNTIES = 10;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		getListView().setOnItemClickListener(new OnItemClickListener(){
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position,
                ...
                CountyList.this.handler.sendMessage(msg);
                //editor.putBoolean("location_service_on", false).commit();
            }});
		
		mAllCounties = (CheckBox) findViewById(R.id.check_all);
		mAllCounties.setChecked(mPreferences.getBoolean("all_parks", false));
		mAllCounties.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            @Override
	...
	protected void onResume()
	{
		...
		mAllCounties.setVisibility(!(mLocManager.isProviderEnabled("gps") || 
	...
	private void setupList()
	{
		...
		if(!(mLocManager.isProviderEnabled("gps") || mLocManager.isProviderEnabled("network"))){
			...
			if(location==null || mAllCounties.isChecked()){
				...
				mCursor = mDatabase.getAreas(VISIBLE_COUNTIES);
			}
		}
			...
			case DIALOG_TIMEOUT_AREA:
				...
				.setPositiveButton(getString(R.string.area_list_retry_button), new DialogInterface.OnClickListener(){
					...
					public void onClick(DialogInterface dialog, int which) {
						CountyList.this.handler.sendEmptyMessage(MESSAGE_RETRY_AREA);
					}})
				.setNegativeButton(android.R.string.cancel, new DialogInterface.OnClickListener(){

					public void onClick(DialogInterface dialog, int which) {
						CountyList.this.handler.sendEmptyMessage(MESSAGE_CANCEL);
					}})
		...
		public void handleMessage(Message msg){
			switch(msg.what){
				case MESSAGE_DOWNLOAD_TAGS:
					...
					if(availableTags == 0){
						TagUpdateThread thread = new TagUpdateThread(CountyList.this, msg.getData().getLong("id"));
						thread.getObservable().addObserver(CountyList.this);
						
						thread.start();
						CountyList.this.showDialog(DIALOG_DOWNLOAD_TAGS);
					}
					else{
						CountyList.this.setResult(RESULT_TAGS_SAME);
						CountyList.this.finish();
					}
					break;
				...
				case MESSAGE_COMPLETE_AREA:
					...
					CountyList.this.setupList();
					
					break;
				case MESSAGE_COMPLETE_TAG:
					dismissDialog(DIALOG_DOWNLOAD_TAGS);
					
					CountyList.this.setResult(RESULT_TAGS_UPDATED);
					CountyList.this.finish();
					
					break;
				case MESSAGE_TIMEOUT_AREA:
					CountyList.this.dismissDialog(DIALOG_DOWNLOAD_AREAS);
					CountyList.this.showDialog(DIALOG_TIMEOUT_AREA);
					break;
				case MESSAGE_TIMEOUT_TAG:
					CountyList.this.dismissDialog(DIALOG_DOWNLOAD_TAGS);
					CountyList.this.showDialog(DIALOG_TIMEOUT_TAG);
					break;
				case MESSAGE_RETRY_AREA:
					CountyList.this.showDialog(DIALOG_DOWNLOAD_AREAS);
					
					AreaUpdateThread thread2 = new AreaUpdateThread(CountyList.this, false, LocationService.getLocation(CountyList.this));
					thread2.getObservable().addObserver(CountyList.this);
					
					thread2.start();
					break;
				case MESSAGE_NO_RESPONSE_TAG:
					CountyList.this.dismissDialog(DIALOG_DOWNLOAD_TAGS);
					CountyList.this.showDialog(DIALOG_NO_RESPONSE_TAG);
					break;
				case MESSAGE_CANCEL:
					CountyList.this.finish();
					break;
